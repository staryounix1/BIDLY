...
const placeholder = (v: string) =>
  v.includes('replace-with') || v.includes('YOUR_') || v.includes('your-');
 ...
 */
export function validateEnv(raw: Record<string, string | undefined>): EnvValidationResult {
  ...
  const errors: string[] = [];
  if (isProd) {
    if (env.DEMO_MODE) errors.push('DEMO_MODE must be false in production');
...
export const env: Env = new Proxy({} as Env, {
  get(_target, prop, receiver) {
    return Reflect.get(getEnv() as object, prop, receiver);
  },
  has(_target, prop) {
    return prop in (getEnv() as object);
  },
  ownKeys() {
